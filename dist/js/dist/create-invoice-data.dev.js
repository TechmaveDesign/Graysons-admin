...
$(function () {
    ...
    $('.invoice-table tbody tr').each(function (i, element) {
      ...
      if (html != '') {
        var qty = $(this).find('.qty').val(),
            ...
            dis_price;
        if ($(this).find('.disc-type').val() == 1) dis_price = price - price * discount / 100;else dis_price = price - discount;
  ...
  $(document).on('keyup change', '.subtotal-table tbody tr td', function () {
    ...
    if ($('.extra-disc-type').val() == 1) subtotal = grosstot - grosstot * ext_discount / 100;else subtotal = grosstot - ext_discount;
